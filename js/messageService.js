angular.module('chatroom').service('messageService', function($http){
  ...
  //Because we're making a POST request, we need a way to tell the server the data we want to give it,
    //in your $http call (along witFh url and method) have a data property which has a value that is equal to another object which a key of message and a value of the message being passed to parse. IE data: {message: yourMessage}
...
this.postMessage = function(message) {
  ...
  this.postCookies = function(message) {
    return $http({
      method: 'POST',
      url: 'http://practiceapi.devmounta.in/api/cookies',
      data: {message: message} 
